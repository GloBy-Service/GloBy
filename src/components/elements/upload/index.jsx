...
const Upload = () => {
  ...
  const onChange = async (e) => {
    ...
    if (f.type !== 'application/pdf' || f.size > 10 * 1024 * 1024) {
      setStatus('error');
      setReasons([f.type !== 'application/pdf' ? 'Invalid file type' : 'File too large (max 10MB)']);
      setFile(f);
      return;
    }
    ...
    setStatus('uploading');
    try {
      const res = await fetch('http://localhost:8080/api/statements/evaluate/pdf', {
        method: 'POST',
        ...
        {status === 'uploading' || status === 'success' || status === 'error' ? (
          <>
            ...
            <span className="status-text">
              {status === 'success' ? 'Upload Successful !' : status === 'uploading' ? 'Uploading...' : 'Upload failed! Please try again.'}
            </span>
            <span className="percent-text">
              {status === 'success' ? '100%' : status === 'error' ? <button onClick={pickFile} className="retry-btn">Try Again</button> : '...'}
            </span>
